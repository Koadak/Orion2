...
if saki=='ai':
    ...
    while not quitgame:
        for event in pygame.event.get():
            ...
            if turn % 2 == 0 and promotion == False:

                turn = turn + 1
                sc = copy.deepcopy(chessBoard.gameTiles)
                y, x, fx, fy = ai2.evaluate(sc)
                m = fy
                n = fx
                if chessBoard.gameTiles[y][x].pieceonTile.tostring() == 'k' or chessBoard.gameTiles[y][
                    x].pieceonTile.tostring() == 'r':
                    chessBoard.gameTiles[y][x].pieceonTile.moved = True
