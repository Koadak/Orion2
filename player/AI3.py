...
class AI:
    ...
    def calculateb(self,gametiles):
        ...
        # Additional value for piece positions
        piece_position_values = {
            ...
            'k': [  # This is for black king, for example in end game position
        ...
        # Invert the values for white pieces, since these are from black's perspective
        for key in ['P', 'N', 'B', 'R', 'Q', 'K']:
            piece_position_values[key] = list(reversed(piece_position_values[key.lower()]))

# Use these updated values in your evaluation function
        ...
        for x in range(8):
            for y in range(8):
                ...
                if piece:
                    ...
                    # Add position value if it's a pawn or knight; add logic for other pieces in a similar way
                    if piece.upper() in piece_position_values:
                        if piece.isupper():
                            # For white pieces (uppercase), the position values are inverted
                            value += piece_position_values[piece.upper()][(7-y) * 8 + x]
                        else:
                            value += piece_position_values[piece.upper()][y * 8 + x]
